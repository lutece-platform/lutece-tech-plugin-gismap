...
parameters.push(parameters['WMS-Base1']=['World_Simple','GeoServer','http://demo.boundlessgeo.com/geoserver/wms','ne:ne']);
parameters.push(parameters['WMS-Base2']=['Otrho_Paris','AGS-IMS','http://capgeo.sig.paris.fr/arcgis/rest/services/','CapGEO_FDP/Ortho_AERODATA_Ete2013']);
parameters.push(parameters['WMS-Layer1']=['2','Plan_Voirie','AGS-MPS','http://capgeo.sig.paris.fr/arcgis/rest/services/','CapGEO_FDP/FDP_CAPGEO_Plan_Voirie_externe',false]);
parameters.push(parameters['WFS1']=['Lieu','AGS','http://capgeo.sig.paris.fr/arcgis/rest/services/capgeo_dasco/DASCO_Evenements_ecoles/FeatureServer/0','EPSG:2154','']);
...
parameters.push(parameters['WFS2']=['Mairie:arrondissements','GeoServer','http://localhost:8080/geoserver/ows?service=WFS&version=1.1.0&request=GetFeature&typeName=Mairie:arrondissements','EPSG:4326','']);
parameters.push(parameters['WFS3']=['DPE_points_noirs','AGS','http://capgeo.sig.paris.fr/arcgis/rest/services/capgeo_dpe/DPE_points_noirs/FeatureServer/6','EPSG:2154','']);
...
parameters.push(parameters['WMTS1']=['Otrho_Paris_Tiled','AGS','http://capgeo.sig.paris.fr/arcgis/rest/services/CapGEO_FDP/Ortho_AERODATA_Ete2013/ImageServer','EPSG:2154',[19.109040301413938,9.554387858775717,4.777061637456608,2.3883985267970536,1.1940669714672763,0.5969011938023876,0.2984505969011938,0.13229193125052918],[-35597500, 48953100]]);
parameters.push(parameters['GeoJSON1'] = ['Test','EPSG:4326']);
parameters.push(parameters['HeatMap1'] = ['HeatMap','1','Lieu',false,'','10','25','20']);
parameters.push(parameters['ThematicSimple1'] = ['Arrondissements','4','Mairie:arrondissements',true, 'c_arinsee', 'mapStyle1', 'defaultStyle1']);
parameters.push(parameters['ThematicComplex1'] = ['Arrondissements_Ideation','5','Mairie:arrondissements',true, 'c_arinsee', 'c_arinsee', 'c_arinsee', 'mapStyle1', 'IdeationStyle', '0.001']);
